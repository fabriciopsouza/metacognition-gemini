...
- **TODA SESSÃO (Mandatório):** Ao iniciar a sessão (`/start-session`), o Agente tem o dever inviolável de verificar a pasta `reports-improve-cross-ai/` em busca de novos relatórios enviados pela IA externa (`claude_to_gemini_*.md`). Se existirem, a leitura crítica é a prioridade zero antes de qualquer código.
