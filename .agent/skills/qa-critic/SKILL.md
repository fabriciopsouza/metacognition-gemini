...
1. **Junction-critic intermediate (J4 — qa-critic → docops):** validação adversarial DENTRO da junção. Critério binário = `APROVADO_LIMPO` (não `_COM_RESSALVAS` nem `REPROVADO`). Iterações ilimitadas até PASS; emendas no mesmo artefato via STATUS-field. Após PASS, forward-only para docops. **Regra Determinística:** `docops` NÃO PODE ser acionado sem que o output registre estritamente a tag de aprovação `[PASS]` ou `APROVADO_LIMPO`.
...
10. **SE** a comunicação, documentação ou log da sessão contém linguagem de "venda", elogio exagerado ao próprio trabalho ou adjetivos bajuladores (ex: "colossal", "fricção reduzida a zero", "maravilhoso", "código perfeito") · **ENTÃO** REPROVADO com instrução: "Remover jargão marqueteiro e sicofante; reescrever em formato métrico, técnico e pericial. Rodar `python tools/hooks/check_sycophancy.py <arquivo>` para confirmar a assepsia." Fonte: Regra 05 (anti-sycophancy).

**Fechamento — Mindset adversarial mantida** para bugs novos não-listados: hipótese default = EXISTE BUG. Estas 10 rules SE/ENTÃO são complemento determinístico (catch recorrente), NÃO substituto da revisão aberta adversarial. Rules emergem de method-audit confirmado (≥2 ocorrências) ou gap isolado high-signal (princípio 11 honesto).
